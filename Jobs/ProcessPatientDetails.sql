...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'ProcessPatientDetails', 
		@enabled=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'ProcessPatientDetails', 
		...
		@command=N'/ISSERVER "\"\SSISDB\AthenaModelProcessing\PatientDetails\Patient_Details_Processing.dtsx\"" /SERVER PPLUSDW /Par "\"$ServerOption::LOGGING_LEVEL(Int16)\"";1 /Par "\"$ServerOption::SYNCHRONIZED(Boolean)\"";True /CALLERINFO SQLAGENT /REPORTING E', 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'ProcessPatientDetails', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'adf2f57f-0069-41d0-8ea0-ef10ba35523c'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
