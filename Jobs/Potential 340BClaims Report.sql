...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Potential 340BClaims Report', 
		@enabled=1, 
...
BEGIN
    ...
    EXEC msdb.dbo.sp_stop_job N''Potential 340BClaims Report'';
END', 
...
Exec Sp_Potential340BClaims @StartDate,@EndDate ', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Generate Excel Without Notes', 
		...
		@command=N'/ISSERVER "\"\SSISDB\Potential340BClaims\Potential340BClaims\Potential340BReportWithoutNotes.dtsx\"" /SERVER PPLUSDW /Par "\"$ServerOption::LOGGING_LEVEL(Int16)\"";1 /Par "\"$ServerOption::SYNCHRONIZED(Boolean)\"";True /CALLERINFO SQLAGENT /REPORTING E', 
		@database_name=N'master', 
		@flags=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Weekdays Daily schedule', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=62, 
		@freq_subday_type=1, 
		...
		@active_start_date=20251212, 
		@active_end_date=99991231, 
		@active_start_time=73000, 
		@active_end_time=235959, 
		@schedule_uid=N'f4c961e8-b3b3-4beb-84f6-4b144d2d5037'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
